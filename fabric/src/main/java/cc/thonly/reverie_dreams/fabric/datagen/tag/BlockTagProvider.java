...
//        pickaxeMineables.add(MIBlocks.COOKTOP);
        RDWoodBlocks.SPIRITUAL_BUNDLE.stream().stream().map(BlockLike::asBlock).forEach(axeMineables::add);
        RDWoodBlocks.LEMON_BUNDLE.stream().stream().map(BlockLike::asBlock).forEach(axeMineables::add);
        RDWoodBlocks.GINKGO_BUNDLE.stream().stream().map(BlockLike::asBlock).forEach(axeMineables::add);
        RDWoodBlocks.PEACH_BUNDLE.stream().stream().map(BlockLike::asBlock).forEach(axeMineables::add);
        RDWoodBlocks.SPIRITUAL_BUNDLE.stream().stream().map(BlockLike::asBlock).forEach(axeMineables::add);

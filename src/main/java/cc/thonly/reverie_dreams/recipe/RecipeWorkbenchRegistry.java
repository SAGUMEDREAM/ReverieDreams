...
public class RecipeWorkbenchRegistry {
    ...
    public static void bootstrap() {
        register("gensokyo_altar", new RecipeWorkbench<>("gensokyo_altar", RDBlocks.GENSOKYO_ALTAR, RecipeManager.GENSOKYO_ALTAR, (registryAccess, recipeId, self) -> {
            ...
            try (ProblemReporter.ScopedCollector logging = new ProblemReporter.ScopedCollector(RDBlockEntityTypes.GENSOKYO_ALTAR::toString, LogUtils.getLogger())) {
                ...
                itemStack.set(DataComponents.BLOCK_ENTITY_DATA, TypedEntityData.of(RDBlockEntityTypes.GENSOKYO_ALTAR, output.buildResult()));
            }
            return itemStack;
        }));
        register("strength_table", new RecipeWorkbench<>("strength_table", RDBlocks.STRENGTH_TABLE, RecipeManager.STRENGTH_TABLE, (registryAccess, recipeId, self) -> {
            Block first = self.getBlock().getFirst();
            ...
            try (ProblemReporter.ScopedCollector logging = new ProblemReporter.ScopedCollector(RDBlockEntityTypes.STRENGTH_TABLE::toString, LogUtils.getLogger())) {
                TagValueOutput output = TagValueOutput.createWithContext(logging, registryAccess);
                ContainerHelper.saveAllItems(output, inventory.items);
                itemStack.set(DataComponents.BLOCK_ENTITY_DATA, TypedEntityData.of(RDBlockEntityTypes.STRENGTH_TABLE, output.buildResult()));
            }
            return itemStack;
        }));
        register("danmaku_crafting_table", new RecipeWorkbench<>("danmaku_crafting_table", RDBlocks.DANMAKU_CRAFTING_TABLE, RecipeManager.DANMAKU_TYPE, (registryAccess, recipeId, self) -> {
            Block first = self.getBlock().getFirst();
            ...
            try (ProblemReporter.ScopedCollector logging = new ProblemReporter.ScopedCollector(RDBlockEntityTypes.DANMAKU_CRAFTING_TABLE::toString, LogUtils.getLogger())) {
                TagValueOutput output = TagValueOutput.createWithContext(logging, registryAccess);
                ContainerHelper.saveAllItems(output, inventory.items);
                itemStack.set(DataComponents.BLOCK_ENTITY_DATA, TypedEntityData.of(RDBlockEntityTypes.DANMAKU_CRAFTING_TABLE, output.buildResult()));
            }
            return itemStack;
        }));
    }

    public static <R extends BaseRecipe> void register(String name, RecipeWorkbench<R> value) {

...
public class RDLootModifies {
    ...
    public static void register() {
        LootTableEvents.MODIFY.register((key, tableBuilder, source, registries) -> {
            ...
            if (key.equals(BLOCKS_SHORT_GRASS)) {
                ...
                for (CropBlockCreator.Instance instance : RDCropBlocks.GRASS_DROPS) {
            ...
            if (RDLootModifies.COMMON_CHESTS.contains(key)) {
                List<CropBlockCreator.Instance> chestDrops = RDCropBlocks.CHEST_DROPS;
                LootPool.Builder poolBuilder = LootPool.lootPool()
    ...
    private static void dropPointPower(LivingEntity entity, DamageSource damageSource) {
        ...
        if ((entity instanceof HairballEntity || entity instanceof Monster || entity instanceof Yousei) && world instanceof ServerLevel serverWorld) {
            ...
            if (random.nextInt(100) < dropChance) {
                entity.spawnAtLocation(serverWorld, new ItemStack(RDItems.POWER, random.nextInt(maxDropCount + 1) + 1));
                entity.spawnAtLocation(serverWorld, new ItemStack(RDItems.POINT, random.nextInt(maxDropCount + 1) + 1));
            }
        }
    ...
    private static void dropIceScales(LivingEntity entity, DamageSource damageSource) {
        ...
        if (world instanceof ServerLevel serverWorld && entity instanceof Stray) {
            ...
            int maxDropCount = 3;
            if (random.nextInt(100) < dropChance) {
                entity.spawnAtLocation(serverWorld, new ItemStack(RDItems.ICE_SCALES, random.nextInt(maxDropCount + 1) + 1));
            }
        }
    }

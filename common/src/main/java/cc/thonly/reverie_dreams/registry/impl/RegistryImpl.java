...
public abstract class RegistryImpl<T> implements WritableRegistry<T> {
    ...
    public RegistryImpl(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle) {
        this.key = key;
    ...
    public RegistryImpl(ResourceKey<? extends Registry<T>> key, @Nullable RegistryImpl<T> parent) {
        this(key, Lifecycle.stable(), parent);
    }

    public RegistryImpl(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle, @Nullable RegistryImpl<T> parent) {
        this(key, lifecycle);
        this.parent = parent;
    }
    ...
    public RegistryImpl<T> syncToClient() {
        this.syncToClient = true;
        return this;
    }

    public <D> RegistryImpl<T> syncToClient(Supplier<RegistrySyncer<T, D>> syncer) {
        this.syncToClient = true;
    ...
    public RegistryImpl<T> shadow() {
        return (RegistryImpl<T>) ReverieDreams.REGISTRY_SHADOWER.apply(this.key, this);
    }
    ...
    @SafeVarargs
    public final RegistryImpl<T> builder(Initialization<T>... initializations) {
    ...
    public RegistryImpl<T> codec(Codec<T> codec) {
        this.codec = codec;
        return this;
    }

    public RegistryImpl<T> defaultId(Identifier defaultId) {
    ...
    public HolderGetter<T> createRegistrationLookup() {
        return new HolderGetter<>() {
            ...
            public Holder.Reference<T> getOrThrow(ResourceKey<T> key) {
                return RegistryImpl.this.getOrCreateEntry(key);
            }
            ...
            public HolderSet.Named<T> getOrThrow(TagKey<T> tag) {
                return RegistryImpl.this.getTag(tag);
            }
        };
    }

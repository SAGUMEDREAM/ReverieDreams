...
public class BiRegistryProvider<K, V> implements Serializable {
    @Getter
    ...
    protected BiRegistryProvider(Identifier key, Class<K> klass, Class<V> vlass) {
        if (key == null) {
            throw new IllegalArgumentException("Registry require a key, but it is null");
        }
        if (BuiltInBiRegistryProviders.ROOT.containsKey(key)) {
        ...
        this.klass = klass;
        this.vlass = vlass;
        BuiltInBiRegistryProviders.ROOT.put(key, this);
    }

    public static <K, V> BiRegistryProvider<K, V> createRegister(Identifier id, Class<K> klass, Class<V> vlass) {
        return new BiRegistryProvider<>(id, klass, vlass);
    }
    ...
    public static <K, V> BiRegistryProvider<K, V> of(Class<K> klass, Class<V> vlass) {
        Set<Map.Entry<Identifier, BiRegistryProvider<?, ?>>> registries = BuiltInBiRegistryProviders.ROOT.entrySet();
        for (Map.Entry<Identifier, BiRegistryProvider<?, ?>> entry : registries) {
            BiRegistryProvider<?, ?> registry = entry.getValue();
            if (registry.klass == klass && registry.vlass == vlass) {
                return (BiRegistryProvider<K, V>) registry;
            }
        }
        return new BiRegistryProvider<>(Identifier.parse(klass.getName().toLowerCase() + "_2_" + vlass.getName().toLowerCase()), klass, vlass);
    }

    public static <K, V> BiRegistryProvider<K, V> ofNullable(Class<K> klass, Class<V> vlass) {
        Set<Map.Entry<Identifier, BiRegistryProvider<?, ?>>> registries = BuiltInBiRegistryProviders.ROOT.entrySet();
        for (Map.Entry<Identifier, BiRegistryProvider<?, ?>> entry : registries) {
            BiRegistryProvider<?, ?> registry = entry.getValue();
            if (registry.klass == klass && registry.vlass == vlass) {
    ...
    public BiRegistryProvider<K, V> reload(ReloadableFactory reloadableFactory) {

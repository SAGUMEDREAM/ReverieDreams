...
public class RDGuiPlaceholderItems {
    public static final List<Holder<Item>> GUI_PLACEHOLDER_ITEM_LIST = new ArrayList<>();
    ...
    public static ItemDelegate registerItem(String name) {
        RegistrySupplier<Item> item = MCBuiltInRegistries.ITEM.register(name, () -> new GuiSlotItem(new Item.Properties().setId(RDItems.keyOf(name))));
        ItemDelegate itemDelegate = ItemDelegate.of(item);
        GUI_PLACEHOLDER_ITEM_LIST.add(itemDelegate);
        return itemDelegate;
    }

    public static List<Holder<Item>> getGuiPlaceholderItemList() {
        return List.copyOf(GUI_PLACEHOLDER_ITEM_LIST);
    }

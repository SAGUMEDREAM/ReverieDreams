...
@Slf4j
@SuppressWarnings({"deprecation", "NullableProblems", "unchecked"})
public abstract class RegistryProvider<T> implements WritableRegistry<T>, RegistryExtension<T>, HolderLookup.RegistryLookup<T> {
    ...
    private final List<RegistryBootstrap<T>> bootstraps = new LinkedList<>();
    ...
    @Nullable
    @Getter
    private RegistryProvider<T> parent;
    @Getter
    private boolean syncToClient = false;
    private RegistrySyncer<T, ?> syncer = null;
    private Supplier<RegistrySyncer<T, ?>> syncerGetter = () -> null;

    public RegistryProvider(ResourceKey<? extends Registry<T>> key) {
        this(key, Lifecycle.stable());
    }

    public RegistryProvider(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle) {
        this.key = key;
    ...
    public RegistryProvider(ResourceKey<? extends Registry<T>> key, @Nullable RegistryProvider<T> parent) {
        this(key, Lifecycle.stable(), parent);
    }

    public RegistryProvider(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle, @Nullable RegistryProvider<T> parent) {
        this(key, lifecycle);
        this.parent = parent;
    }
    ...
    public void validate() {
        ...
        this.keyToEntry.forEach((registryKey, reference) -> {
            if (registryKey == null) {
                log.error("Can't validate registry key, rawId: {}", next.get());
                return;
            }
            if (reference == null) {
                log.error("Can't validate registry entry reference, registryKey: {}", registryKey);
                return;
            }
            if (reference.value() == null) {
                log.error("Can't validate registry entry value, registryKey: {}", registryKey);
                return;
            }
    ...
    public RegistryProvider<T> syncToClient() {
        this.syncToClient = true;
        return this;
    }

    public <D> RegistryProvider<T> syncToClient(Supplier<RegistrySyncer<T, D>> syncer) {
        this.syncToClient = true;
        this.syncerGetter = (Supplier<RegistrySyncer<T, ?>>) (Object) syncer;
        return this;
    }

    public <D> RegistrySyncer<T, D> getSyncer(Class<D> dClass) {
        return this.getSyncer();
    }

    public <D> RegistrySyncer<T, D> getSyncer() {
        if (this.syncer == null) {
            this.syncer = this.syncerGetter.get();
        }
        return (RegistrySyncer<T, D>) this.syncer;
    }

    public RegistryProvider<T> shadow() {
        Optional<RegistryProviderShadowerFactory> registryProviderShadowerFactoryOptional = PlatformProxies.REGISTRY_PROVIDER_SHADOWER_FACTORY;
        if (registryProviderShadowerFactoryOptional.isPresent()) {
            RegistryProviderShadowerFactory registryProviderFactory = registryProviderShadowerFactoryOptional.get();
            return (RegistryProvider<T>) registryProviderFactory.apply(this.key, this);
        }
        return null;
    }
    ...
    public RegistryProvider<T> build() {
        if (this.frozen) {
            return this;
        }
        this.bootstraps.forEach(step -> step.bootstrap(this));
        this.freeze();
        return this;
    }

    @SafeVarargs
    public final RegistryProvider<T> builder(RegistryBootstrap<T>... bootstrap) {
        this.bootstraps.addAll(Arrays.asList(bootstrap));
        return this;
    }

    @SafeVarargs
    public final RegistryProvider<T> reloadBuilder(RegistryReloadStep<T>... steps) {
        this.reloadableSteps.addAll(Arrays.asList(steps));
        this.reloadable = true;
        return this;
    }

    public RegistryProvider<T> codec(Codec<T> codec) {
        this.codec = codec;
        return this;
    }

    public RegistryProvider<T> defaultId(Identifier defaultId) {
        this.defaultId = defaultId;
        return this;
    }

    public ResourceKey<T> createKey(Identifier id) {
        return ResourceKey.create(this.key, id);
    }
...
//            log.warn("Attempted to unregister non-existing key {}", key);
    ...
    public void unboundTag() {
        this.tags.clear();
        this.keyToEntry.forEach((key, reference) -> {
            reference.tags = new HashSet<>();
        });
    }
    ...
    public void bindTags(Map<TagKey<T>, List<Holder<T>>> pendingTags) {
        ...
        Map<Holder.Reference<T>, Set<TagKey<T>>> map = new Object2ObjectLinkedOpenHashMap<>();
        ...
        map.forEach((reference, tagKeys) -> {
            HolderReferenceAccessor<T> accessor = (HolderReferenceAccessor<T>) reference;
            accessor.reverie_dreams$bindTags(tagKeys);
        });
    }
    ...
    public HolderGetter<T> createRegistrationLookup() {
        return new HolderGetter<>() {
            ...
            public Holder.Reference<T> getOrThrow(ResourceKey<T> key) {
                return RegistryProvider.this.getOrCreateEntry(key);
            }
            ...
            public HolderSet.Named<T> getOrThrow(TagKey<T> tag) {
                return RegistryProvider.this.getTag(tag);
            }
        };
    }
    ...
    public @Nullable T getValue(@Nullable ResourceKey<T> key) {
        Holder.Reference<T> entry = this.keyToEntry.get(this.resolveKey(key));
        if (entry == null) {
    ...
    public @Nullable T getValue(@Nullable Identifier id) {
        ...
        Holder.Reference<T> ref = this.idToEntry.get(this.resolveId(id));
        if (ref != null) {
            return ref.value();
        }
        return this.getAny().map(Holder::value).orElse(null);
    ...
    public Optional<RegistrationInfo> registrationInfo(ResourceKey<T> key) {
        return Optional.ofNullable(this.keyToEntryInfo.get(this.resolveKey(key)));
    }
    ...
    Holder.Reference<T> getOrCreateEntry(ResourceKey<T> key) {
        return this.keyToEntry.computeIfAbsent(this.resolveKey(key), (key2) -> Holder.Reference.createStandAlone(this, key2));
    }
    ...
    public Optional<Holder.Reference<T>> get(Identifier id) {
        return Optional.ofNullable(this.idToEntry.get(this.resolveId(id)));
    }
    ...
    public PendingTags<T> prepareTagReload(TagLoader.LoadResult<T> tags) {
        return new PendingTags<T>() {
            ...
            public ResourceKey<? extends Registry<? extends T>> key() {
                return RegistryProvider.this.key();
            }

            @Override
            public RegistryLookup<T> lookup() {
                return RegistryProvider.this;
            }
            ...
            @Override
            public int size() {
                return RegistryProvider.this.size();
            }

            public Map<TagKey<T>, List<Holder<T>>> contents() {
                Map<TagKey<T>, HolderSet.Named<T>> tags = RegistryProvider.this.tags;

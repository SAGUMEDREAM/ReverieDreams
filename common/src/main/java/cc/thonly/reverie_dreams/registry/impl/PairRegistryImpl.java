...
public class PairRegistryImpl<K, V> implements Serializable {
    @Getter
    ...
    protected PairRegistryImpl(Identifier key, Class<K> klass, Class<V> vlass) {
        if (key == null) {
            throw new IllegalArgumentException("Registry require a key, but it is null");
        }
        if (PairRegistryImpls.ROOT.containsKey(key)) {
        ...
        this.klass = klass;
        this.vlass = vlass;
        PairRegistryImpls.ROOT.put(key, this);
    }

    public static <K, V> PairRegistryImpl<K, V> createRegister(Identifier id, Class<K> klass, Class<V> vlass) {
        return new PairRegistryImpl<>(id, klass, vlass);
    }

    public static <K, V> PairRegistryImpl<K, V> of(Identifier id, Class<K> klass, Class<V> vlass) {
        return (PairRegistryImpl<K, V>) PairRegistryImpls.ROOT.computeIfAbsent(id, x -> new PairRegistryImpl<>(id, klass, vlass));
    }

    public static <K, V> PairRegistryImpl<K, V> of(Class<K> klass, Class<V> vlass) {
        Set<Map.Entry<Identifier, PairRegistryImpl<?, ?>>> registries = PairRegistryImpls.ROOT.entrySet();
        for (Map.Entry<Identifier, PairRegistryImpl<?, ?>> entry : registries) {
            PairRegistryImpl<?, ?> registry = entry.getValue();
            if (registry.klass == klass && registry.vlass == vlass) {
                return (PairRegistryImpl<K, V>) registry;
            }
        }
        return new PairRegistryImpl<>(Identifier.parse(klass.getName().toLowerCase() + "_2_" + vlass.getName().toLowerCase()), klass, vlass);
    }

    public static <K, V> PairRegistryImpl<K, V> ofNullable(Class<K> klass, Class<V> vlass) {
        Set<Map.Entry<Identifier, PairRegistryImpl<?, ?>>> registries = PairRegistryImpls.ROOT.entrySet();
        for (Map.Entry<Identifier, PairRegistryImpl<?, ?>> entry : registries) {
            PairRegistryImpl<?, ?> registry = entry.getValue();
            if (registry.klass == klass && registry.vlass == vlass) {
    ...
    public PairRegistryImpl<K, V> reload(ReloadableFactory reloadableFactory) {

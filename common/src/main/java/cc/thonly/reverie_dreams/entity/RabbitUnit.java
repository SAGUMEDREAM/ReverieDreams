...
public class RabbitUnit extends BaseNPCLikeEntity implements Leashable, FriendlyFaction, VariantData, YouseiType {
    ...
    public RabbitUnit(EntityType<? extends TamableAnimal> entityType, Level world) {
        super(entityType,
                world,
                (
        ...
        if (ReverieDreams.RD.nextBoolean()) {
            ...
            if (ReverieDreams.RD.nextBoolean()) {
                this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(RDItems.SILVER_SWORD.asItem()));
            } else {
                this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(RDItems.SILVER_SPEAR.asItem()));
            }
    ...
    @Override
    public void tick() {
        this.setSkinType(this.variant != null ? this.variant.getSkinType() : RabbitUnitVariants.RABBIT_UNIT_0.getSkinType());
        if (this.getTarget() instanceof RabbitUnit) {
            this.setTarget(null);
        }
        super.tick();
    }

...
@ToString
public class RoleCard implements SerializableProvider<RoleCard>, RegistryEntryOwnerBindable<RoleCard>, BuiltinObject {
    public static final Codec<RoleCard> CODEC = UnitCodec.unit(RoleCard::new);
    public static final Long DEFAULT_COLOR = 16777215L;
    @Setter
    @Getter
    private RegistryProvider<RoleCard> owner;
    @Setter
    @Getter
    private Identifier id;
    private Identifier itemId;
    private Long color = DEFAULT_COLOR;
    private final List<NPCRoleType> entries = new LinkedList<>();

    private RoleCard() {
    }

    public RoleCard(Identifier id, Long color, List<NPCRoleType> roles) {
        this.id = id;
        this.color = color;
        this.of(roles);
    }

    public RoleCard(Identifier id, Long color, NPCRoleType... roles) {
        this.id = id;
        this.color = color;
        this.of(roles);
    }
    ...
    @Nullable
    public static RoleCard findRoleCard(NPCRoleType role) {
        Stream<RoleCard> stream = BuiltInRegistryProviders.ROLE_CARD.values().stream().filter(card -> card.entries.contains(role));
        return stream.findFirst()
                .orElse(null);
    }

    public RoleCard of(List<NPCRoleType> roles) {
        for (NPCRoleType role : roles) {
    ...
    public RoleCard of(NPCRoleType... roles) {
    ...
    public ItemStackTemplate getTemplate() {
        ...
        ItemStackTemplateHelper.modify(template, (template1, modifier) -> {
            ...
            modifier.set(RDDataComponentTypes.ROLE_CARD_ID.value(), this.getId());
        });
        return template;
    }

    public Stream<NPCRoleType> stream() {
        return this.entries.stream();
    }

    public Optional<NPCRoleType> random() {
    ...
    public RoleCard build() {
        ...
        ReverieDreams.COMMON_LATE_INIT.add(() -> {
            for (NPCRoleType entry : this.entries) {

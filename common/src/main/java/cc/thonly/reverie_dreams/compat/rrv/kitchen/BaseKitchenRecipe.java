...
public class BaseKitchenRecipe implements ReliableClientRecipe {
    ...
    public BaseKitchenRecipe(Identifier id, KitchenRecipe recipe, BaseKitchenClientRecipeType recipeType) {
        this.id = id;
        this.recipe = recipe;
    ...
    public void bindSlots(RecipeViewMenu.SlotFillContext context) {
        for (int i = 0; i < 5; i++) {
            if (i > this.inputs.size() - 1) {
                SlotContent slotContent = SlotContent.of(RDGuiPlaceholderItems.EMPTY_ITEM.value());
        ...
        context.bindSlot(6, SlotContent.of(RDGuiPlaceholderItems.PROGRESS_TO_RESULT_DOWN.value()));
    }

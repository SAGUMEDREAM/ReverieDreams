...
public class LilyPadBlockMixin implements BonemealableBlock {
    ...
    public void applyGrowth(Level world, RandomSource random, BlockPos pos, BlockState state) {
        ...
        if (chance < 25) {
            itemEntity = new ItemEntity(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, new ItemStack(RDIngredientItems.LOTUS_NUTS.asItem(), number), 0, 0.2, 0);
        } else if (chance < 45) {
            itemEntity = new ItemEntity(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, new ItemStack(RDIngredientItems.TWIN_LOTUS.asItem(), number), 0, 0.2, 0);
        } else if (chance < 65) {

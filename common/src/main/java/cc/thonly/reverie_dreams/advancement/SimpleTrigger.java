...
public class SimpleTrigger extends SimpleCriterionTrigger<SimpleTrigger.Condition> {
    ...
    public static Criterion<Condition> of(Identifier powerHandCrank) {
        return RDCriteriaTriggers.SIMPLE_TRIGGER.value().createCriterion(new Condition(powerHandCrank));
    }

    public static void trigger(ServerPlayer player, Identifier identifier) {
        RDCriteriaTriggers.SIMPLE_TRIGGER.value().trigger(player, (condition) -> {
    ...
    public record Condition(Identifier location) implements SimpleInstance {
        public static final Codec<Condition> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(Identifier.CODEC.fieldOf("trigger").forGetter(Condition::identifier)).apply(instance, Condition::new);
        });

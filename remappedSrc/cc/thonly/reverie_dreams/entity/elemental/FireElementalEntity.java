...
public class FireElementalEntity extends BaseNPCLikeEntity implements ElementalMob {
    ...
    public FireElementalEntity(EntityType<? extends TamableAnimal> entityType, Level world) {
    ...
    public boolean hurtServer(ServerLevel world, DamageSource source, float amount) {
        ...
        if (source.type().equals(damageSources.inFire().type())) {
            return false;
        }
    ...
    public void tick() {
        ...
        if (!world.isClientSide() && world instanceof ServerLevel serverWorld) {
            this.addEffect(new MobEffectInstance(MobEffects.FIRE_RESISTANCE, 3 * 20));
            this.setRemainingFireTicks(20);
            if (this.aTick > 10) {
                boolean bl1 = this.isInWater();
                boolean bl2 = serverWorld.getFluidState(this.blockPosition()).is(Fluids.LAVA) ||
                        serverWorld.getBlockState(this.blockPosition()).is(Blocks.FIRE);
